...
public class JRStyledText implements Cloneable {
     ...
     *
     */
    @Property(valueType = Boolean.class, defaultValue = PropertyConstants.BOOLEAN_FALSE, scopes = {PropertyScope.CONTEXT, PropertyScope.REPORT}, sinceVersion = PropertyConstants.VERSION_3_6_1)
    public static final String PROPERTY_AWT_IGNORE_MISSING_FONT = JRPropertiesUtil.PROPERTY_PREFIX + "awt.ignore.missing.font";

    @Property(valueType = Boolean.class, scopes = {PropertyScope.GLOBAL}, sinceVersion = PropertyConstants.VERSION_3_1_3)
    ...
    private static final boolean AWT_SUPERSCRIPT_FIX_ENABLED = JRPropertiesUtil.getInstance(DefaultJasperReportsContext.getInstance()).getBooleanProperty(PROPERTY_AWT_SUPERSCRIPT_FIX_ENABLED);
...
//				}
            ...
            while (runLimit < iterator.getEndIndex() && (runLimit = iterator.getRunLimit(FONT_ATTRS)) <= iterator.getEndIndex()) {
                ...
                Font awtFont = fontUtil.getAwtFontFromBundles(fontAttribute, ((TextAttribute.WEIGHT_BOLD.equals(attrs.get(TextAttribute.WEIGHT)) ? Font.BOLD : Font.PLAIN) | (TextAttribute.POSTURE_OBLIQUE.equals(attrs.get(TextAttribute.POSTURE)) ? Font.ITALIC : Font.PLAIN)), (Float) attrs.get(TextAttribute.SIZE), locale, ignoreMissingFont);
                if (awtFont == null) {
    ...
    public JRStyledText cloneText() {
        try {
            ...
            runs.forEach(r -> clone.runs.add(r.cloneRun()));
            return clone;
        } catch (CloneNotSupportedException e) {
            // never
            throw new JRRuntimeException(e);
    ...
    private void getChars(int srcBegin, int srcEnd, char[] dst, int dstBegin) {
        if (text != null) {
            ...
            // should not happen
            throw new JRRuntimeException(EXCEPTION_MESSAGE_KEY_CANNOT_COPY_CHARACTERS, new Object[]{srcBegin, srcEnd});
        }
    }
}
